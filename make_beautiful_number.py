...
def make_beautiful_number():
    ...
    try:
        ...
        target_orders = destination_session.query(updated_orders).filter(
                        updated_orders.c.upd_number.is_(None))
        for order in target_orders:
            upd_number = phonenumbers.parse(
                         order.contact_phone, "RU").national_number
            inserted_number = updated_orders.update().\
                values(upd_number=upd_number).\
